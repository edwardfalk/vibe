...
class GameState {
    ...
    checkLevelProgression() {
        if (this.score >= this.nextLevelThreshold) {
            ...
            console.log(`🎉 LEVEL UP! Now level ${this.level}. Next level at ${this.nextLevelThreshold} points (need ${nextLevelIncrease} more)`);
    ...
    updateGameOverTimer() {
        if (this.gameState === 'gameOver') {
            ...
            if (window.testMode && this.gameOverTimer >= 60) {
                console.log('🔄 Auto-restarting game in test mode for continuous testing');
                this.restart();
            }
        }
    }
