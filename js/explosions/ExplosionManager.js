...
class EnemyFragmentExplosion {
    ...
    update() {
        ...
        if (this.timer >= this.maxTimer && this.fragments.length === 0 && 
    ...
    draw(p) {
        ...
        for (const particle of this.centralExplosion.particles) {
            ...
            // Glow effect
            if (particle.glow > 0) {
                fill(red(particle.color), green(particle.color), blue(particle.color), alpha * particle.glow * 0.3);
                noStroke();
        ...
        for (const fragment of this.fragments) {
            ...
            fill(red(fragment.color), green(fragment.color), blue(fragment.color), alpha);
            noStroke();
            ...
            if (fragment.type === 'head' || fragment.type === 'helmet') {
                ...
                rect(-fragment.size * 0.3, -fragment.size * 0.1, fragment.size * 0.6, fragment.size * 0.2);
            }
...
export class ExplosionManager {
    ...
    addBombDebrisField(centerX, centerY, count = 5) {
        ...
        console.log(`☢️ Created radioactive debris field with ${count} contamination zones`);
    }
