...
export class BaseEnemy {
    ...
    constructor(x, y, type, config, p, audio) {
        ...
        const speechConfig = CONFIG.SPEECH_SETTINGS[type.toUpperCase()] || CONFIG.SPEECH_SETTINGS.DEFAULT;
    ...
    getGlowColor(isSpeaking) {
        if (this.type === 'tank') {
            return isSpeaking ? this.p.color(150, 100, 255) : this.p.color(100, 50, 200);
        } else if (this.type === 'rusher') {
            return isSpeaking ? this.p.color(255, 150, 200) : this.p.color(255, 100, 150);
        } else if (this.type === 'stabber') {
            return isSpeaking ? this.p.color(255, 200, 50) : this.p.color(255, 140, 0);
        } else {
            // Grunt default
            return isSpeaking ? this.p.color(100, 255, 100) : this.p.color(50, 200, 50);
        }
    }
    ...
    draw(p = this.p) {
        ...
        if (typeof p.frameCount !== 'undefined' && p.frameCount % 30 === 0 && CONFIG.GAME_SETTINGS.DEBUG_COLLISIONS) {
            console.log(`[ENEMY DRAW] type=${this.type} x=${this.x.toFixed(1)} y=${this.y.toFixed(1)} health=${this.health} visible=${this.isVisible ? this.isVisible : 'n/a'}`);
        }
    ...
    drawHealthBar(p) {
        if (this.health < this.maxHealth && !this.markedForRemoval) {
            ...
            if (CONFIG.GAME_SETTINGS.DEBUG_COLLISIONS) {
                console.log(`[ENEMY DEBUG] drawHealthBar: type=${this.type} health=${this.health} maxHealth=${this.maxHealth} at (${this.x.toFixed(1)},${this.y.toFixed(1)})`);
            }
            // Background bar
            p.fill(100, 100, 100);
            ...
            const healthColor = healthPercent > 0.5 ? p.color(0, 255, 0) : 
                               healthPercent > 0.25 ? p.color(255, 255, 0) : p.color(255, 0, 0);
            p.fill(healthColor);
            ...
            if (this.health <= 0) {
                p.stroke(255, 0, 0);
                p.strokeWeight(3);
                p.line(this.x - barWidth / 2, barY, this.x + barWidth / 2, barY + barHeight);
                p.line(this.x + barWidth / 2, barY, this.x - barWidth / 2, barY + barHeight);
                p.noStroke();
            }
        }
    ...
    takeDamage(amount, bulletAngle = null, damageSource = null) {
        if (CONFIG.GAME_SETTINGS.DEBUG_COLLISIONS) {
            console.log(`[DEBUG] Grunt takeDamage called: health=${this.health} markedForRemoval=${this.markedForRemoval}`);
        }
        this.health -= amount;
        this.hitFlash = 8;
        if (this.health <= 0) {
