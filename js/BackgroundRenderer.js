...
} from './systems/background/BeatReactiveBackground.js';
import { drawBeatPulseOverlay } from './systems/background/BeatPulseOverlay.js';
...
export class BackgroundRenderer {
  ...
  drawCosmicAuroraBackground(p = this.p) {
    ...
    for (let i = 0; i < gradientSteps; i++) {
      ...
      // Cosmic aurora gradient colors - enhanced with beat reactivity
      let r, g, b;
      if (inter < 0.3) {
        ...
        // Purple to cosmic blue - more vibrant on beat
        const t = p.map(inter, 0.3, 0.7, 0, 1);
        r = p.lerp(25, 20, t);
        g = p.lerp(15, 30, t);
        b = p.lerp(45, 65, t);
        // Add blue shift on strong beats
        b += downbeatIntensity * 20;
        g += beatIntensity * 10;
      } else {
        // Cosmic blue to deep purple - shifts toward magenta on beats
  ...
  drawEnhancedSpaceElements(p = this.p) {
    ...
    // Enhanced aurora wisps with beat-reactive pulsing
    const auroraBeatPulse = window.beatClock
      ? window.beatClock.getBeatIntensity(6) * 20
      : 0;
  ...
  drawInteractiveBackgroundEffects(p = this.p) {
    ...
    drawBeatPulseOverlay(p, window.beatClock);
