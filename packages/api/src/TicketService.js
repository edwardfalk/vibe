...
export class TicketService {
  ...
  async list(query) {
    this.logger.info(`[Service] Listing tickets with query: ${JSON.stringify(query)}`);
    try {
        const options = {
  ...
  async create(ticketData) {
    ...
    try {
        ...
        try {
            ...
            throw new HttpError(`Ticket with ID ${ticketData.id} already exists.`, 400);
        } catch (e) {
  ...
  async update(id, updates) {
    ...
    try {
        ...
        const allowedFields = [
          'type', 'title', 'status', 'tags', 'checklist', 'artifacts', 'relatedTickets', 'slug', 'description'
        ];
        
        const changes = [];
        for (const key in updates) {
            if (Object.prototype.hasOwnProperty.call(updates, key) && allowedFields.includes(key)) {
