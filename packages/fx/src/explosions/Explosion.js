...
export class Explosion {
  constructor(x, y, type = 'enemy') {
    ...
    for (let i = 0; i < particleCount; i++) {
      ...
      if (type === 'tank-plasma') {
        vxRange = [-2, 2];
        vyRange = [-2, 2]; // Reduced from [-3, 3]
        sizeRange = [4, 10];
        lifeRange = [50, 70]; // Reduced size and life
      } else if (type === 'rusher-explosion') {
        vxRange = [-8, 8];
        vyRange = [-8, 8]; // Increased from [-5, 5] for more dramatic spread
        sizeRange = [6, 18];
        lifeRange = [40, 80]; // Increased size and life for much more visibility
      } else if (type === 'grunt-bullet-kill') {
        vxRange = [-3, 3];
        vyRange = [-3, 3]; // Reduced from [-5, 5]
        sizeRange = [2, 5];
        lifeRange = [20, 35]; // Much smaller and shorter
      } else if (type === 'grunt-plasma-kill') {
        vxRange = [-3, 3];
        ...
        vxRange = [-4, 4];
        vyRange = [-4, 4]; // Reduced from [-9, 9], [-6, 6]
        sizeRange = [3, 8];
        lifeRange = [20, 35]; // Reduced
      } else if (type === 'rusher-plasma-kill') {
        vxRange = [-4, 4];
        vyRange = [-4, 4]; // Reduced from [-6, 6]
        sizeRange = [3, 8];
        lifeRange = [25, 40]; // Reduced
      } else if (type === 'tank-bullet-kill') {
        vxRange = [-3, 3];
        vyRange = [-3, 3]; // Reduced from [-4, 4], [-3, 5]
        sizeRange = [4, 10];
        lifeRange = [40, 60]; // Reduced
      } else if (type === 'tank-plasma-kill') {
        vxRange = [-4, 4];
        vyRange = [-4, 4]; // Reduced from [-5, 5]
        sizeRange = [5, 12];
  ...
  draw(p) {
    ...
    if (this.hasShockwave && this.shockwaveRadius > 0) {
      p.stroke(
        ...
        255,
        255,
        100 * (1 - this.shockwaveRadius / this.maxShockwaveRadius)
      );
      p.strokeWeight(3);
      p.noFill();
