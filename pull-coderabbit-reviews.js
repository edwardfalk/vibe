...
async function fetchCodeRabbitReviews() {
  ...
  try {
    ...
    const prResponse = await retryOperation(
      async () => {
    ...
    for (const pr of prs) {
      ...
      if (reviewsResponse.ok) {
        ...
        if (coderabbitReviews.length > 0) {
          console.log(
            `   🤖 Found ${coderabbitReviews.length} CodeRabbit reviews`
          );
          ...
          for (const review of coderabbitReviews) {
            ...
            if (suggestions.length > 0) {
              ...
              if (highPriority.length > 0) {
                console.log(
                  `   🚨 ${highPriority.length} high-priority suggestions:`
                );
                highPriority.forEach((s) => {
                  console.log(
                    `      - ${s.category}: ${s.text.substring(0, 80)}...`
                  );
                });
              }
            }
...
function categorizeIssue(content) {
  ...
  if (
    lowercaseContent.includes('security') ||
  ...
  if (
    lowercaseContent.includes('style') ||
...
if (
  ...
  import.meta.url.includes('pull-coderabbit-reviews.js')
) {
  fetchCodeRabbitReviews();
}

export { fetchCodeRabbitReviews };
